...
class VehicleAddVC: UIViewController,UIImagePickerControllerDelegate , UINavigationControllerDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let vehicleRegistrationNumber: String = userDefault.object(forKey: RegistrationFinalKeys.kVehicleRegistrationNo) as? String {
            txtVehicleRegistrationNumber.text = vehicleRegistrationNumber
        }
                                                
        if let vehicleClass: String = userDefault.object(forKey: RegistrationFinalKeys.kCompanyModel) as? String {
            txtCompany.text = vehicleClass
        }
                                                   
        if let carType: String = UserDefaults.standard.object(forKey: RegistrationFinalKeys.kCarThreeTypeName) as? String {
            txtCarType.text = carType
        }
        
        if let data = userDefault.object(forKey: RegistrationFinalKeys.kVehicleImage) as? Data {
            if let decodedData = NSKeyedUnarchiver.unarchiveObject(with: data) {
                if let imgageVehicle: UIImage = UIImage(data: decodedData as! Data) {
                    imgVehicle.image = imgageVehicle
                }
            }
        }
    }
    override func viewDidLayoutSubviews() {
        ...
        imgVehicle.layer.cornerRadius = imgVehicle.frame.height/2
    ...
    func setLocalizable() {
        ...
        txtVehicleRegistrationNumber.placeholder = "Vehicle Registration Number".localized
        txtCompany.placeholder = "Car Model".localized
    ...
    func webserviceforGetCarModels() {
        ...
        webserviceForVehicalModelList("" as AnyObject) { (result, status) in
            ...
            if (status)
            {
                ...
                {
                    let dataOFCars = self.aryDataCarsAndTaxi[i]
...
//    }
    func isValidateForNext() -> (Bool,String) {
        ...
        let currentImage : Data = imgVehicle.image!.pngData()!
        let imgPlaceholder : Data = UIImage(named: "iconCARPlaceholder")!.pngData()!
        //        let sb = Snackbar()
        
        if txtVehicleRegistrationNumber.text?.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines) == "" {
            isValidate = false
            validatorMessage = "Please enter vehicle registration number".localized
            ...
            isValidate = false
            validatorMessage = "Please enter car model".localized
            ...
            isValidate = false
            validatorMessage = "Please select car type".localized
            //            sb.createWithAction(text: "Please select vehicle type".localized, actionTitle: "Dismiss".localized, action: { print("Button is push") })
            //            sb.show()
        }
        else if currentImage.elementsEqual(imgPlaceholder) || imgVehicle.image == nil //UIImage.init(named: "") //imgVehicle.image == UIImage(named: "iconCARPlaceholder")
        {
            isValidate = false
    ...
    func setData() {
        ...
        let vehicleClasscarType = txtCarType.text //change for passing vehicle ID not name
        //            let VehiclaMakeCompanyModel = txtVehicleMake.text
        //            let vehiclePassenger = txtNumberPassenger.selectedItem
        let vehicleID = userDefault.value(forKey: RegistrationFinalKeys.kVehicleClass)
        ...
        userDefault.set(vehicleID, forKey: RegistrationFinalKeys.kVehicleClass)
        userDefault.set(VehiclaCompanyModelName, forKey: RegistrationFinalKeys.kCompanyModel)
        //            userDefault.set(vehiclePassenger, forKey: RegistrationFinalKeys.kNumberOfPasssenger)
        
    }
    @IBAction func btnNext(_ sender: Any)
    {
//        self.performSegue(withIdentifier: "fourthComplete", sender: self)
        ...
        if validator.0 == true {
            setData()
            userDefault.set("5", forKey: RegistrationFinalKeys.kPageNo)
            self.performSegue(withIdentifier: "fourthComplete", sender: self)
...
// MARK: Delegates
extension VehicleAddVC : UITextFieldDelegate {
    ...
    //            UIImagePickerController.availableMediaTypes(for: .photoLibrary)!
                                          ...
                                          didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any])
        {
            ...
            if let pickedImage = info[UIImagePickerController.InfoKey.originalImage] as? UIImage {
                imgVehicle.contentMode = .scaleAspectFill
